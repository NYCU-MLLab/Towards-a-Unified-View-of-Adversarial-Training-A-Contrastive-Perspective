...
class ResNet(nn.Module):
    def __init__(self, block, layers, bn_names, use_normalize=True, num_classes=1000, zero_init_residual=False,
    ...
    def encoder(self, x, bn_name=None):
        # debug
    ...
    def _forward_impl(self, x, bn_name=None, cluster=False):

        feat = self.encoder(x, bn_name)
        
        if isinstance(self.fc, proj_head):
            out = self.fc(feat, bn_name)
        else:
            out = self.fc(feat)
        
        if cluster:
            return (self.fc_002(feat), self.fc_010(feat), self.fc_050(feat), self.fc_100(feat), self.fc_500(feat))
        else:
            return out

    def forward(self, x, bn_name=None, cluster=False):
        return self._forward_impl(x, bn_name, cluster)
