...
from models.resnet_rocl import ResNet18 as rocl_resnet18
...
parser.add_argument('--rocl', action='store_true',
                    help='if specified, eval a rocl model')

parser.add_argument('--advcl', action='store_true',
                    help='if specified, eval a advcl model')
...
parser.add_argument('--aa', action='store_true', help="if specified, use Auto-Attacks")


parser.add_argument('--eval_only', action='store_true', help="if specified, evaluate all checkpoints")

parser.add_argument('--no_adv_test', action='store_true', help="if specified, only evaluate on clean data")
...
def eval_adv_test(encoder, linear, device, test_loader, epsilon, alpha, criterion, log, attack_iter=40):
    ...
    model = nn.Sequential(encoder, linear)
    ...
    log.info(' * AA Robust Accuracy {top1.avg}'.format(top1=top1))
...
def get_state_dict():
    ...
    checkpoint = torch.load(os.path.join(model_dir, 'rob_best_model.pt'))
...
def main():
    ...
    if args.rocl:
        encoder = rocl_resnet18(with_fc=False).to(device)
    elif args.advcl:
        encoder = resnet18(with_fc=False, use_normalize=False).to(device)
    else:
    ...
    _, _, _, _, best_acc, _, starting_epoch, _, best_rob_acc = get_train_stats()
    
    print('evaluating checkpoint, epoch: ', starting_epoch, ' best_acc: ', best_acc, ' best_rob_acc: ', best_rob_acc)
    ...
    log.info("On the rob_best_model (epoch {}), test acc is {}, test AA robust acc is {}".format(starting_epoch + 1, best_acc, vali_rob_acc))
